...
In Copilot CLI, the pattern is: be selective. Type `/context` to see your token budget. The GitHub MCP server and Chrome DevTools MCP (Tip #13) earn their context cost. But "install every MCP server that looks cool" is a trap.

And remember: if a CLI tool can do the same job (Tip #14), it costs zero context until the agent calls `--help`.
